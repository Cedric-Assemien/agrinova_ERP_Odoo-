...
class DepotMobile(models.Model):
    _name = "mobile.depot"
    _description = "Dépôt Mobile Money"
    ...
    def _check_operateur(self):
        """
        Bloque la sauvegarde si l'opérateur reste indéterminé
        """
        for rec in self:
            if not rec.operateur:
                raise ValidationError("Numéro invalide : aucun opérateur reconnu.")

    @api.constrains('numero')
    def _check_numero_format(self):
        """
        Validation du format du numéro de téléphone (10 chiffres, commence par 01, 04 ou 07)
        """
        for rec in self:
            if rec.numero:
                # Nettoyage du numéro
    ...
    def create(self, vals_list):
        ...
        for record in records:
            if record.operateur and record.reference == "Nouveau":
                prefix = prefix_map.get(record.operateur, 'DEP/XX/')
